...
def run_ultimate_battle(
    ...
    # [수정된 부분: Eco-Mode Parameter Tuning]
    # 구조 변경 없이 파라미터만으로 에너지를 줄이는 최적값
    aegis_params=dict(
        base_gain=3.5,     # 기존 4.0 -> 3.5 (평상시 힘 빼기)
        ki=0.15,           # 기존 0.3 -> 0.15 (급발진 방지, 에너지 절약의 핵심)
        gamma=0.005,       # 기존 0.008 -> 0.005 (더 신중하게 게인 변경)
        target_noise=0.8,
        gain_max=8.0       # 기존 15.0 -> 8.0 (최대 파워 제한)
    ),
    ...
    if verbose:
        print("⚔️  STARTING ULTIMATE CONTROL BATTLE (Eco-Tuning) ⚔️")
        print(f"Strategy: AEGIS base_gain={aegis.base_gain}, gain_max={aegis.gain_max} vs Filtered PID")
...
# ==========================================
# 1. Controller & Environment Classes (기존 로직 유지)
# ==========================================
class FilteredPIDController:
    def __init__(self, kp, ki, kd, tau_f=0.1, out_min=0.0, out_max=100.0, dt=1.0):
        self.kp = kp
...
def generate_anova_dataset(n_samples=30, duration=500):
    ...
    """
    print(f"🚀 Generating data for ANOVA (N={n_samples} trials)...")
...
def main():
    ...
    if p_val < 0.05:
        print(">> 결론: P < 0.05 이므로 통계적으로 유의미한 차이가 있음 (H1 채택)")
        print(f">> AEGIS가 PID와 성능이 '확실히 다르다'는 것이 증명됨.")
    else:
        print(">> 결론: 통계적으로 유의미한 차이가 없음 (H0 기각 실패)")
    ...
    print("\n✅ Boxplot saved to 'anova_boxplot.png'")
    plt.show()
...
def run_full_analysis(n_samples=30):
    print(f"🚀 [Step 1] 몬테카를로 시뮬레이션 시작 (N={n_samples})...")
    ...
    # ---------------------------------------------------------
    # [Step 2] 통계 분석 (Rigorous Statistics)
    # ---------------------------------------------------------
    print("\n" + "="*60)
    print("      🧪 엄밀한 통계 검증 보고서 (Rigorous Statistical Report)")
    print("="*60)
    ...
    print(f"[1] 정규성 검정 (Shapiro-Wilk): p={shapiro_p:.4e}")
    ...
    print(f"[2] 가설 검정 ({test_name})")
    print(f"    * Statistic: {stat:.4f}")
    print(f"    * P-Value  : {p_val:.4e}")

    if p_val < 0.05:
        print("    >> ✨ 결과: 통계적으로 매우 유의미한 차이 있음 (Significant)")
    else:
        print("    >> ☁️ 결과: 유의미한 차이 없음")

    # 4. 효과 크기 (Cohen's d)
    mean_diff = np.mean(diff)
    std_diff = np.std(diff, ddof=1)
    cohens_d = mean_diff / std_diff
    print(f"[3] 효과 크기 (Cohen's d): {cohens_d:.4f}")
    if cohens_d > 0.8: print("    >> 해석: 'Huge Effect' (압도적 차이)")

    # 5. 제어 에너지 분석
    mean_e_pid = df['energy_pid'].mean()
    mean_e_aegis = df['energy_aegis'].mean()
    e_ratio = mean_e_aegis / mean_e_pid
    print(f"[4] 제어 에너지 효율성 (Energy Efficiency)")
    print(f"    * Energy Ratio (AEGIS/PID): {e_ratio:.2f}")
    if e_ratio < 1.0: print("    >> ✅ AEGIS가 에너지를 더 적게 사용함 (효율적)")
    elif e_ratio < 1.2: print("    >> ⚠️ AEGIS가 에너지를 약간 더 사용함 (허용 범위)")
    else: print("    >> 🚨 AEGIS가 과도한 에너지를 사용함")
